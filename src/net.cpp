...
void ThreadGetMyExternalIP(void* parg)
{
    ...
    RenameThread("GULFCOINGOLD-ext-ip");
...
static const char *strDNSSeed[][2] = {
    {"198.187.30.69", "198.187.30.69"},
	{"162.243.223.207", "162.243.223.207"},
	{"69.105.102.5", "69.105.102.5"},
	{"166.143.5.10", "166.143.5.10"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("GULFCOINGOLD-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("GULFCOINGOLD-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("GULFCOINGOLD-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("GULFCOINGOLD-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. GULFCOINGOLD is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode(void* parg)
{
    ...
    RenameThread("GULFCOINGOLD-start");
