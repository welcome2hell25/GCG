...
#else
    const char* pszModule = "GULFCOINGOLD";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GULFCOINGOLD
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GULFCOINGOLD
    // Mac: ~/Library/Application Support/GULFCOINGOLD
    // Unix: ~/.GULFCOINGOLD
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MYGULFCOINGOLD";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "GULFCOINGOLD.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "GULFCOINGOLDd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong GULFCOINGOLD will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("GULFCOINGOLD"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
