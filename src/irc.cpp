...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("GULFCOINGOLD-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GULFCOINGOLDTEST2\r");
            Send(hSocket, "WHO #GULFCOINGOLDTEST2\r");
        } else {
            // randomly join #GULFCOINGOLD00-#GULFCOINGOLD05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #GULFCOINGOLD%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GULFCOINGOLD%02d\r", channel_number).c_str());
        }
